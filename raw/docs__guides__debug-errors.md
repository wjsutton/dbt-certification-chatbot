...
4. Get comfortable with compiled files and the logs.
    ...
    - **core users**: Note that your code editor _may_ be hiding these files from the tree view [VSCode help](https://stackoverflow.com/questions/42891463/how-can-i-show-ignored-files-in-visual-studio-code)).
...
## Runtime Errors
_Note: If you're using the studio_ide to work on your project, you're unlikely to encounter these errors._
...
```
  ...
  - Then execute `open /Users/alice/.dbt` (adjusting accordingly), and check that you have a `profiles.yml` file. If you do not have one, set one up using [these docs](https://docs.getdbt.com/docs/local/profiles.yml)
...
_Note: if you're using the studio_ide to work on your dbt project, this error often shows as a red bar in your command prompt as you work on your dbt project. For core users, these won't get picked up until you run `dbt run` or `dbt compile`._
...
To prevent this:
- _(core only)_ Use snippets to auto-complete pieces of Jinja ([atom-dbt package](https://github.com/dbt-labs/atom-dbt))
...
To prevent this:
- (core users) Turn on indentation guides in your code editor to help you inspect your files
...
1. Open the offending file:
    ...
    - **core:** Open the model as above. Also open the compiled SQL (in this case `target/run/jaffle_shop/models/customers.sql` as per the error message) — it can be useful to show these side-by-side in your code editor.
2. Try to re-execute the SQL to isolate the error:
    - **dbt:** Use the `Preview` button from the model file
    - **core:** Copy and paste the compiled query into a query runner (e.g. the Snowflake UI, or a desktop app like DataGrip / TablePlus) and execute it
3. Fix the mistake.
...
- **core**: Open the `logs/dbt.log` file.
...
We've all been there. dbt uses the last-saved version of a file when you execute a command. In most code editors, and in the studio_ide, a dot next to a filename indicates that a file has unsaved changes. Make sure you hit `cmd + s` (or equivalent) before running any dbt commands — over time it becomes muscle memory.

### Editing compiled files
_(More likely for core users)_
